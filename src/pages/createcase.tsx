...
const ClientDetailsForm: React.FC<ClientDetailsFormProps> = ({
  ...
  const regions = ['Windsor', 'Leamington', 'Harrow', 'Amherstburg', 'Tilbury', 'Chatham'];
  const statuses = ['Open', 'Ongoing', 'Close'];

  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await axios.get('https://55c1-184-147-92-214.ngrok-free.app/client', {
          headers: {
    ...
    const fetchStaff = async () => {
      try {
        const response = await axios.get('https://55c1-184-147-92-214.ngrok-free.app/user', {
          headers: {
            'ngrok-skip-browser-warning': 'true' // Add this header
          }
        });
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get('https://55c1-184-147-92-214.ngrok-free.app/service', {
          headers: {
            'ngrok-skip-browser-warning': 'true' // Add this header
          }
        });
  ...
  const handleSubmit = async () => {
    ...
    const selectedClientData = clients.find(client => client.fullName === selectedClient);
    console.log("hola")
    console.log(selectedService)
  
    const selectedServiceData = services.find(service => service.name === selectedService);
    const selectedStaffData = staffList.find(staff => staff.name === selectedStaff);
    
    console.log(selectedClientData?.fullName,selectedServiceData?.name,selectedStaffData)
    if (selectedClientData && selectedServiceData && selectedStaffData) {
      ...
      const url = "https://55c1-184-147-92-214.ngrok-free.app/case/";
  ...
  const InputField: React.FC<InputFieldProps> = ({ label, value, isDate = false, onChange }) => {
    return (
      <div className="flex flex-col mt-16 w-full text-sm font-medium text-white max-md:mt-10">
        <label htmlFor={label.toLowerCase().replace(' ', '-')} className="self-start">
          {label}
        </label>
        <div className="flex gap-5 justify-between py-3 pr-5 pl-2 mt-2.5 rounded-lg bg-zinc-600">
          ...
          <img 
            src={isDate ? "https://cdn.builder.io/api/v1/image/assets/TEMP/29d95f28c63baf9c902e478c6e96d543b413099df8333570e2db4882f14dfd1c?placeholderIfAbsent=true&apiKey=877b457759d54d259ca44608a2613044" : "https://cdn.builder.io/api/v1/image/assets/TEMP/6de4bb2988311f02b00733baab499042405b6490526453f604364c8e13c554b7?placeholderIfAbsent=true&apiKey=877b457759d54d259ca44608a2613044"} 
            alt="Calendar Icon" 
          />
        </div>
      </div>
  ...
  const DropdownField: React.FC<DropdownFieldProps> = ({ label, value, onChange, options }) => {
    return (
      <div className="flex flex-col mt-16 w-full text-sm font-medium text-white">
        ...
        <select value={value} onChange={onChange} className="bg-zinc-600 py-3 rounded-lg mt-2 text-white">
  ...
  return (
    <form className="flex overflow-hidden flex-col px-9 pt-6 pb-9 rounded-2xl bg-zinc-700 max-w-[867px] max-md:px-5" onSubmit={handleSubmit}>
        ...
        <div className="flex gap-5 max-md:flex-col">
          <div className="flex flex-col w-[33%] max-md:ml-0 max-md:w-full">
            <DropdownField 
              ...
              options={clients.map(client => ({ label: client.fullName, value: client.fullName }))}
            />
            <DropdownField 
              label="Region" 
          ...
          <div className="flex flex-col ml-5 w-[33%] max-md:ml-0 max-md:w-full">
            <DropdownField 
              ...
              options={staffList.map(staff => ({ label: staff.name, value: staff.name }))} 
            />
            <DropdownField 
              label="Status" 
